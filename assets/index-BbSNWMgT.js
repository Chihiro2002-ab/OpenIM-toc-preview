const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/OpenIMApp-B_5DEDJl.js","assets/OpenIMApp-CO1zNyWQ.css","assets/PreviewShell-BF3zPPT0.js","assets/PreviewShell-sPp8tC5P.css"])))=>i.map(i=>d[i]);
...
`};window.mountOpenIMIcons=(t=document,n={})=>{let r=n.aliases||{},i=new Set(n.flipped||[]);t.querySelectorAll(`[data-openim-icon]`).forEach(t=>{let n=t.dataset.openimIcon,a=r[n]||n,o=e[a];if(!o){console.warn(`Missing OpenIM icon: ${n} -> ${a}`);return}if(t.matches(`svg.openim-icon`)&&t.dataset.openimResolved===a){t.classList.toggle(`is-flipped`,i.has(n));return}let s=document.createElement(`template`);s.innerHTML=o.trim();let c=s.content.firstElementChild,l=[...t.classList].filter(e=>e!==`openim-icon`&&e!==`is-flipped`);c.classList.add(`openim-icon`,...l),c.classList.toggle(`is-flipped`,i.has(n)),c.dataset.openimIcon=n,c.dataset.openimResolved=a,c.setAttribute(`aria-hidden`,`true`),c.setAttribute(`focusable`,`false`),t.replaceWith(c)})}})(),document.documentElement.lang=le===`en`?`en`:`zh-CN`,document.title=ce?`OpenIM`:`OpenIM React 客户端`,document.querySelector(`meta[name="description"]`)?.setAttribute(`content`,ce?`OpenIM desktop and mobile client preview`:`OpenIM React 客户端桌面端与移动端预览`),(0,S.createRoot)(document.querySelector(`#root`)).render((0,C.jsx)(oe,{}));export{w as a,o as c,u as d,D as i,c as l,le as n,x as o,ce as r,h as s,ue as t,d as u};